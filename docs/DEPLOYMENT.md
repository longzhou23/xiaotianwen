...
- SnowLuma 镜像：`motricseven7/snowluma:latest`
...
目标主机需要 Docker Compose。部署或执行 `deploy/update.sh` 时会拉取 AstrBot 与 SnowLuma 的官方 `latest` 镜像；普通重启不会拉取。迁移后检查 `snowluma-live/compose.yml` 中的数据路径、目标用户 UID/GID，以及 `.env` 文件权限。`.env`、QQ 数据、AstrBot 配置和日志可能包含凭据或隐私数据，不应公开。
