...
class ChatPlus(Star):
    ...
    @staticmethod
    def _split_system_prompt_injection(
        plugin_name: str,
        text: str,
    ) -> tuple[str, str]:
        """Split a third-party system injection into stable and runtime parts.

        Group Chat Plus must preserve the fixed AtTool format contract in the
        stable system prompt, while the per-request permission result belongs
        in a temporary user-content part. Other third-party system additions
        are treated as runtime context because they are not known to be stable.
        """
        normalized = ChatPlus._normalize_prompt_text(text)
        if not normalized:
            return "", ""

        if plugin_name != "astrbot_plugin_AtTool":
            return "", normalized
        ...
        stable = normalized[:split_at].rstrip()
        runtime = normalized[split_at:].lstrip()
    ...
    def _build_stable_system_prompt(
        ...
        rewrite_result: Any,
        per_plugin_injections: Any,
    ) -> tuple[str, list[str]]:
        """Build a stable system prompt and collect dynamic system additions.

        The plugin-owned persona remains the stable base. Third-party system
        additions are not blindly re-merged into that base: AtTool's fixed
        format contract is retained, while permission/runtime additions and
        other dynamic plugin text are returned for temporary user-content
        injection. If tracking data is unavailable, preserve the old rewrite
        result as a compatibility fallback rather than dropping instructions.
        """
        base = ChatPlus._normalize_prompt_text(plugin_system_prompt)
        fallback = ChatPlus._normalize_prompt_text(fallback_system_prompt)
        rewritten = ChatPlus._normalize_prompt_text(
            getattr(rewrite_result, "merged_system_prompt", "")
        )

        if not base or not isinstance(per_plugin_injections, dict):
            return rewritten or fallback or base, []

        stable_parts = [base]
        stable_seen = {base}
        runtime_parts: list[str] = []

        for plugin_name, injection in per_plugin_injections.items():
            if not isinstance(injection, dict):
                continue
            raw_text = injection.get("system_prompt_added", "")
            stable_text, runtime_text = ChatPlus._split_system_prompt_injection(
                str(plugin_name),
                str(raw_text or ""),
            )
            stable_text = ChatPlus._normalize_prompt_text(stable_text)
            runtime_text = ChatPlus._normalize_prompt_text(runtime_text)

            if stable_text and stable_text not in stable_seen:
                stable_seen.add(stable_text)
                stable_parts.append(stable_text)
            if runtime_text:
                runtime_parts.append(
                    "[临时插件注入 - "
                    f"{plugin_name}]\n{runtime_text}\n"
                    f"[临时插件注入 - {plugin_name} 结束]"
                )

        return "\n\n".join(stable_parts), runtime_parts

    async def _apply_global_system_prompt_split(
        self,
        ...
        """为所有 LLM 请求统一拆分动态 system_prompt 注入。

        Group Chat Plus 自有请求仍由下方的完整恢复逻辑处理；未带本插件标记的
        请求也经过这里收尾，避免 AtTool、反注入和其他第三方插件的实时文本留在
        system_prompt 中。prompt、contexts 和已有 extra_user_content_parts 不改写。
        """
        baseline = event.get_extra("_gcp_request_baseline", None)
        if isinstance(baseline, dict):
            baseline_system_prompt = str(baseline.get("system_prompt") or "")
        else:
            baseline_system_prompt = ""

        current_system_prompt = str(req_snapshot.get("system_prompt") or "")
        if not baseline_system_prompt:
            baseline_system_prompt = current_system_prompt

        if not isinstance(per_plugin_injections, dict):
            # 没有逐插件追踪时保留原请求，避免丢失安全指令。
            return

        stable_system_prompt, runtime_blocks = self._build_stable_system_prompt(
            plugin_system_prompt=baseline_system_prompt,
            fallback_system_prompt=current_system_prompt,
            rewrite_result=None,
            per_plugin_injections=per_plugin_injections,
        )

        # 如果某个 handler 用替换而不是前后追加的方式改写了 system_prompt，
        # 追踪器可能无法归属它；把未识别部分也转成运行时块，避免静默丢失。
        tracked_system_additions = [
            self._normalize_prompt_text(
                str(item.get("system_prompt_added") or "")
            )
            for item in per_plugin_injections.values()
            if isinstance(item, dict) and item.get("system_prompt_added")
        ]
        if current_system_prompt != baseline_system_prompt and not tracked_system_additions:
            unmatched = current_system_prompt
            if current_system_prompt.startswith(baseline_system_prompt):
                unmatched = current_system_prompt[len(baseline_system_prompt) :]
            elif current_system_prompt.endswith(baseline_system_prompt):
                unmatched = current_system_prompt[: -len(baseline_system_prompt)]
            unmatched = self._normalize_prompt_text(unmatched)
            if unmatched:
        ...
        req.system_prompt = stable_system_prompt
        if runtime_blocks:
            runtime_prompt_text = "\n\n".join(
                block for block in runtime_blocks if block
            )
            try:
                extra_parts = getattr(req, "extra_user_content_parts", None)
                if isinstance(extra_parts, list):
                    runtime_part = TextPart(text=runtime_prompt_text)
                    mark_as_temp = getattr(runtime_part, "mark_as_temp", None)
                    if callable(mark_as_temp):
                        mark_as_temp()
                    extra_parts.append(runtime_part)
                else:
                    # 兼容旧框架：没有动态内容块时保留原有 system 注入。
                    req.system_prompt += "\n\n" + runtime_prompt_text
            except Exception as exc:
                logger.warning(
                    f"⚠️ 全局动态 system_prompt 转移失败，回退到 system_prompt：{exc}"
                )
                req.system_prompt += "\n\n" + runtime_prompt_text

        if self.debug_mode:
            logger.info(
                "  ✅ 全局请求已完成稳定/动态 system_prompt 分层: "
                f"动态块={len(runtime_blocks)}, system长度={len(req.system_prompt)}, "
                f"extra数量={len(getattr(req, 'extra_user_content_parts', []) or [])}"
            )
    ...
    def _merge_per_plugin_prompt(
        ...
        for plugin_name, injections in per_plugin_injections.items():
            ...
            # system_prompt_added 会在后续拆分为稳定 system 内容和临时
            # extra_user_content_parts；extra_parts_added 已由框架保留，
        ...
        for handler in handlers:
            ...
            async def _make_tracking_wrapper(
                ...
                **kwargs,
                ...
                # 标准 OnLLMRequestEvent 是 (event, req)，但主动对话/部分
                # 兼容链路偶尔会把原始消息事件重复放进 args，形成
                # (event, message_event, req)。不能假设 args[0] 就是请求对象。
                req = None
                for candidate in args:
                    try:
                        _ = candidate.contexts
                        req = candidate
                        break
                    except Exception:
                        continue
                ...
                # 第一层 wrapper 执行前保存请求基线，供最终的全局分层 hook 使用。
                # 该基线位于第三方插件注入之前，不受后续动态内容污染。
                try:
                    if event.get_extra("_gcp_request_baseline", None) is None:
                        event.set_extra(
                            "_gcp_request_baseline",
                ...
                # 第三方 on_llm_request 的标准签名是 (event, req)。仅传递
                # 已识别的请求对象，避免把兼容链路里重复的消息事件继续传入。
                await __original(event, req, **kwargs)
                ...
                try:
                    ...
                    extra_added = [item for item in extra_after if item not in extra_before]
                except Exception:
                    extra_added = []
    ...
    async def on_llm_request(self, event: AstrMessageEvent, req: ProviderRequest):
        ...
        未带本插件标记的请求也会进入全局 system_prompt 分层，仅移动可追踪的
        动态 system 注入，不改写其 prompt、contexts 或已有 extra 内容。
        ...
        if not is_plugin_request:
            ...
            per_plugin_injections = event.get_extra(
                "_gcp_per_plugin_injections", None
            )
            await self._apply_global_system_prompt_split(
                event,
                req,
                req_snapshot,
            ...
            event.set_extra("_gcp_request_baseline", None)
            return
        ...
        # 使用 hasattr 而非 getattr，因为 Pydantic 模型字段可能存在但值为 None
        ...
        # 🔧 稳定 system_prompt / 动态注入分层：
        # 人格、固定 AtTool 规范和固定回复规则留在稳定 system_prompt；
        # 当前权限、情绪和其他第三方实时 system 注入改为临时用户内容。
        # 若逐插件追踪不可用，helper 会保留旧的 rewrite_result 作为兼容回退。
        stable_system_prompt, dynamic_system_prompt_blocks = (
            self._build_stable_system_prompt(
                plugin_system_prompt=plugin_system_prompt,
                fallback_system_prompt=req_snapshot.get("system_prompt", "") or "",
                rewrite_result=rewrite_result,
                per_plugin_injections=per_plugin_injections,
            )
        )
        req.system_prompt = stable_system_prompt
        if dynamic_system_prompt_blocks and self.debug_mode:
            logger.info(
                "  ✅ 已将动态 system_prompt 注入移至临时用户内容: "
                f"{len(dynamic_system_prompt_blocks)} 段"
            )
        ...
        # P1: 第三方动态 system 注入、工具清单和情绪提示均是每轮变化的运行时信息。
        # 通过临时用户上下文承载，避免破坏 system prompt 的缓存前缀。
        # 静态工具规范、工具调用指引与工具 schema 仍保持原有 system/tool 位置。
        runtime_prompt_blocks: list[str] = []
        runtime_prompt_blocks.extend(dynamic_system_prompt_blocks)
        smart_batch_hint = event.get_extra(
            "_group_chat_plus_smart_batch_hint", None
        )
        if smart_batch_hint:
        ...
        if mood_hint:
            runtime_prompt_blocks.append(str(mood_hint))
            if self.debug_mode:
                logger.info(f"  ✅ 已追加情绪提示到临时用户内容")

        if runtime_prompt_blocks:
            runtime_prompt_text = "\n\n".join(
                block for block in runtime_prompt_blocks if block
            )
            try:
                extra_parts = getattr(req, "extra_user_content_parts", None)
                if isinstance(extra_parts, list):
                    ...
                    # 兼容旧框架：不支持动态内容块时维持原来的 system 注入。
                    req.system_prompt += "\n\n" + runtime_prompt_text
            except Exception as e:
                logger.warning(f"⚠️ 注入运行时上下文失败，回退到 system_prompt：{e}")
                req.system_prompt += "\n\n" + runtime_prompt_text
        ...
        try:
            ...
            event.set_extra("_gcp_per_plugin_injections", None)
            event.set_extra("_gcp_request_baseline", None)
            event.set_extra("_group_chat_plus_smart_batch_hint", None)
