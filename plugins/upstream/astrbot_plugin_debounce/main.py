...
_MEDIA_COMPONENT_TYPES = (Image, Record, Video, File)
_DEBOUNCE_BYPASS_KEY = "_debounce_bypass_event"


def _event_is_debounce_bypassed(event: Optional[AstrMessageEvent]) -> bool:
    """同一轮 Agent/工具调用复用事件对象时，避免再次进入防抖。"""
    if event is None:
        return False
    try:
        return bool(event.get_extra(_DEBOUNCE_BYPASS_KEY, False))
    except Exception:
        return False


def _mark_event_debounce_bypassed(event: Optional[AstrMessageEvent]) -> None:
    if event is None:
        return
    try:
        event.set_extra(_DEBOUNCE_BYPASS_KEY, True)
    except Exception:
        pass


def _event_has_media(event: Optional[AstrMessageEvent]) -> bool:
    """判断事件是否包含图片、语音、视频或文件等媒体组件。"""
    if event is None:
        return False
    try:
        components = event.message_obj.message or []
    except Exception:
        return False
    return any(isinstance(component, _MEDIA_COMPONENT_TYPES) for component in components)
...
class MessageBuffer:
    ...
    def add(self, message: str, event: AstrMessageEvent = None):
        ...
        if event:
            # 如果图片事件后紧跟文字，保留包含媒体的事件，确保合并后的伪造事件仍带上原图。
            # 连续媒体事件则使用最新事件；纯文字之间仍使用最新事件。
            if (
                self.event is None
                or _event_has_media(event)
                or not _event_has_media(self.event)
            ):
                self.event = event
...
class DebouncePlugin(Star):
    ...
    async def on_waiting_llm_request(self, event: AstrMessageEvent):
        ...
        # 同一轮 Agent 的工具/跟进请求会复用伪造事件，首轮通过后必须全部放行。
        if _event_is_debounce_bypassed(event):
            return
        ...
        # 只有开启“收到新消息时取消回复”时，才丢弃当前回复并合并新消息。
        # 关闭时保留当前回复，新消息在当前请求结束后独立处理。
        cancel_on_new_message = bool(
            self.config.get("cancel_on_new_message", True)
        )
        if session_id in self.pending_llm_sessions and cancel_on_new_message:
            self.discard_next_response.add(session_id)
            logger.debug(
                f"[Debounce] 新消息到达，取消旧LLM回复并准备合并: {session_id}"
            )
    ...
    async def on_llm_request(self, event: AstrMessageEvent, req: ProviderRequest):
        ...
        # 空文本的纯媒体事件也必须进入防抖窗口，否则后续“看看这个”会被拆成第二轮请求。
        # 没有文本且没有媒体的事件仍按原逻辑忽略。
        if not message_text and not has_media:
            return
        ...
        if msg_id in self.skip_debounce_msg_ids:
            ...
            # 标记正在处理LLM请求，并把该事件标记为本轮已通过防抖。
            # Tool loop / follow-up 请求会复用这个事件，不能再次进入防抖。
            self.pending_llm_sessions.add(session_id)
            _mark_event_debounce_bypassed(event)
            logger.debug(f"[Debounce] 伪造消息直接通过并放行后续工具请求: {session_id}")
            return

        # 关闭开关时，不能把新消息拦截进当前正在运行的 LLM 请求。
        # 直接放行，让 AstrBot 在当前回复完成后独立处理它。
        if session_id in self.pending_llm_sessions and not cancel_on_new_message:
            logger.debug(
                f"[Debounce] cancel_on_new_message=false，保留旧回复并独立放行新消息: {session_id}"
            )
            return
        ...
        # 这样可以确保连续多条消息都能正确合并
        should_merge = (
            session_id in self.waiting_sessions
            or (
                cancel_on_new_message
                and session_id in self.pending_llm_sessions
            )
        )
        ...
        if should_merge:
            ...
            # Use a quiet window for every message in an active burst.
            if session_id in self.monitor_tasks:
                self.monitor_tasks[session_id].cancel()
                del self.monitor_tasks[session_id]
            ...
            if session_id not in self.monitor_tasks:
                task = asyncio.create_task(
                    self._monitor_session(session_id, timeout_seconds)
                )
                self.monitor_tasks[session_id] = task
            logger.debug(
                f"[Debounce] message buffered in quiet window: {session_id}, "
                f"count={len(buffer.messages)}"
            )
            return
        ...
        # Always wait for a quiet window before the first LLM request.
        # This makes the plugin debounce complete short messages as well.
        self.waiting_sessions.add(session_id)
        event.stop_event()
        ...
        self.monitor_tasks[session_id] = task
        logger.debug(
            f"[Debounce] first message buffered in quiet window: {session_id}"
        )
        return
    ...
    async def on_llm_response(self, event: AstrMessageEvent, resp: LLMResponse):
        ...
        # 只有开关仍然开启时才丢弃旧回复；关闭后保留旧回复。
        cancel_on_new_message = bool(
            self.config.get("cancel_on_new_message", True)
        )
        if session_id in self.discard_next_response and cancel_on_new_message:
