...
class ErosThanatosPlugin(Star):
    ...
    async def inject_emotion_prompt(self, event: AstrMessageEvent, req):
        ...
        # 这些数值随对象与回合变化；作为临时上下文可保留 system prompt 缓存前缀。
        # 若框架不支持 extra_user_content_parts，则保守回退为原 system 注入。
        try:
            extra_parts = getattr(req, "extra_user_content_parts", None)
            if isinstance(extra_parts, list):
                emotion_part = TextPart(text=emotion_prompt)
                mark_as_temp = getattr(emotion_part, "mark_as_temp", None)
            ...
            logger.warning(f"[ErosThanatos] 注入动态情绪上下文失败，回退到 system_prompt：{e}")
